  ...
  <div class="inner">
    ...
    <div class="sig-row">
      <div class="sig-box">
        ...
        <div class="sig-name">NANTO s.r.o.</div>
        <div class="sig-role">Radek Kunčický, jednatel</div>
        <div class="sig-line"></div>
        <div style="font-size:9px;color:var(--muted);">Podpis a razítko</div>
      </div>
      <div class="sig-box">
        <div class="sig-label">Za objednatele</div>
        <div class="sig-name">{{klient_jmeno}}</div>
        <div class="sig-role">Datum: ________________________</div>
        <div class="sig-line"></div>
        <div style="font-size:9px;color:var(--muted);">Podpis</div>
      </div>
    </div>
  </div>
  <div class="page-footer">
...
async function main() {
  console.log('Seeding NANTO HTML templates...')

  const templateEntries = [
    { key: 'KLIMA', label: 'Klimatizace – NANTO', html: HTML_KLIMA, technologie: 'KLIMA' },
    { key: 'TEPELNE_CERPADLO', label: 'Tepelné čerpadlo – NANTO', html: HTML_TEPELNE_CERPADLO, technologie: 'TEPELNE_CERPADLO' },
    { key: 'PODLAHOVE_TOPENI', label: 'Podlahové vytápění – NANTO', html: HTML_PODLAHOVE_TOPENI, technologie: 'PODLAHOVE_TOPENI' },
    { key: 'REKUPERACE', label: 'Rekuperace – NANTO', html: HTML_REKUPERACE, technologie: 'REKUPERACE' },
    { key: 'VZDUCHOTECHNIKA', label: 'Vzduchotechnika – NANTO', html: HTML_VZDUCHOTECHNIKA, technologie: 'VZDUCHOTECHNIKA' },
    { key: 'JINE', label: 'Obecná nabídka – NANTO', html: HTML_JINE, technologie: 'JINE' },
  ]

  const templateIds: Record<string, string> = {}

  for (const entry of templateEntries) {
    // Find existing template for this org by name
    const existing = await prisma.quoteTemplate.findFirst({
      where: { orgId: NANTO_ORG_ID, nazev: entry.label }
    })

    let template
    if (existing) {
      template = existing
      console.log(`Found existing template: ${entry.label} (${template.id})`)
    } else {
      ...
      console.log(`Created template: ${entry.label} (${template.id})`)
    }
    templateIds[entry.key] = template.id

    // Upsert QuoteTemplateHtml
    const existingHtml = await prisma.quoteTemplateHtml.findUnique({
      where: { templateId: template.id }
    })
    if (existingHtml) {
      await prisma.quoteTemplateHtml.update({
        where: { templateId: template.id },
        data: { htmlContent: entry.html }
      })
      console.log(`Updated HTML for: ${entry.label}`)
    } else {
      await prisma.quoteTemplateHtml.create({
        data: { templateId: template.id, htmlContent: entry.html }
      })
      console.log(`Created HTML for: ${entry.label}`)
    }
  }

  // Upsert OrgTemplateMapping for each technology
  for (const entry of templateEntries) {
    const templateId = templateIds[entry.key]
    const existing = await prisma.orgTemplateMapping.findUnique({
      where: { orgId_technologie: { orgId: NANTO_ORG_ID, technologie: entry.technologie } }
    })
    if (existing) {
      await prisma.orgTemplateMapping.update({
        where: { id: existing.id },
        data: { templateId }
      })
      console.log(`Updated mapping: ${entry.technologie} → ${templateId}`)
    } else {
      await prisma.orgTemplateMapping.create({
        data: { orgId: NANTO_ORG_ID, technologie: entry.technologie, templateId }
      })
      console.log(`Created mapping: ${entry.technologie} → ${templateId}`)
    }
  }

  // Set singleTemplate=false for NANTO org
  const orgSettings = await prisma.orgSettings.findUnique({ where: { orgId: NANTO_ORG_ID } })
  if (orgSettings) {
    await prisma.orgSettings.update({
      where: { orgId: NANTO_ORG_ID },
      data: { singleTemplate: false }
    })
    console.log('Set singleTemplate=false for NANTO org')
  } else {
    await prisma.orgSettings.create({
      data: { orgId: NANTO_ORG_ID, singleTemplate: false }
    })
    console.log('Created OrgSettings with singleTemplate=false for NANTO org')
  }
