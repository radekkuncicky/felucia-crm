...
const HTML_SOD_12PCT = `<!DOCTYPE html>
<html lang="cs">
<head>
<meta charset="UTF-8">
<style>
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  /* ---------- Záloha ---------- */
  .deposit-box { border: 1px solid #E6E6EC; border-radius: 5px; overflow: hidden; margin: 8px 0 10px; }
  .deposit-row { display: flex; align-items: stretch; }
  .deposit-row + .deposit-row { border-top: 1px solid #E6E6EC; }
  .deposit-key {
    width: 180px; flex-shrink: 0; padding: 11px 16px;
    font-size: 10.5px; font-weight: 600; color: #757584;
    text-transform: uppercase; letter-spacing: 0.7px;
    background: #F9F9FB; border-right: 1px solid #E6E6EC;
  }
  .deposit-val { padding: 11px 16px; font-size: 13px; font-weight: 700; color: #15151F; }
  .deposit-val.hl { color: #FFC93C; font-size: 14.5px; }
  ...
  @media print {
    ...
    .art, .sub, .party, .price, .deposit-box, .sign { page-break-inside: avoid; }
  }
</style>
</head>
<body>
<div class="page">
  ...
  <div class="sec">
    ...
    <div class="art">
      ...
      <div class="art-body">Cenu Díla se Objednatel zavazuje uhradit následujícím způsobem:</div>
    </div>

    <div class="deposit-box">
      <div class="deposit-row">
        <div class="deposit-key">Záloha</div>
        <div class="deposit-val hl">{{hodnota_zalohy}}</div>
      </div>
      <div class="deposit-row">
        <div class="deposit-key">Splatnost zálohy</div>
        <div class="deposit-val">{{zaloha_splatnost}} dní od doručení zálohové faktury</div>
      </div>
    </div>

    <div class="sub">
      <div class="sub-num">3.3.1.</div>
      <div class="sub-body">Záloha na cenu Díla ve výši <strong>{{hodnota_zalohy}}</strong> je splatná do <strong>{{zaloha_splatnost}} dní</strong> od doručení zálohové faktury vystavené Zhotovitelem.</div>
    </div>
    <div class="sub">
      <div class="sub-num">3.3.2.</div>
      <div class="sub-body">Doplatek zbývající části ceny Díla bude Zhotovitelem Objednateli vyúčtován po dokončení Díla se splatností závěrečné faktury <strong>14 dní</strong>.</div>
    </div>
  </div>
  ...
  <div class="sec">
    ...
    <div class="art">
      ...
      <div class="art-body">V případě prodlení Objednatele s úhradou zálohy na cenu Díla či ceny Díla, vzniká Zhotoviteli vůči Objednateli právo na zaplacení smluvní pokuty ve výši <strong>0,03 %</strong> z dlužné částky za každý, byť jen započatý den prodlení.</div>
    </div>
  ...
  <div class="sec">
    ...
    <div class="art">
      ...
      <div class="art-body">Zhotovitel není povinen převzít Staveniště ani/nebo zahájit provádění Díla před úplným zaplacením zálohy na cenu Díla. V případě prodlení Objednatele se zaplacením zálohy se odpovídajícím způsobem prodlužuje termín dokončení Díla a Zhotovitel sdělí Objednateli nový termín převzetí Staveniště či nový termín zahájení provádění Díla. Takové prodloužení termínu dokončení není prodlením Zhotovitele.</div>
    </div>
  </div>
...
async function main() {
  await prisma.contractTemplate.upsert({
    where: { id: 'sod-12pct-nanto-v1' },
    create: {
      id: 'sod-12pct-nanto-v1',
      orgId: NANTO_ORG_ID,
      nazev: 'SOD 12 % DPH — se zálohou',
      popis: 'Smlouva o dílo se sníženou sazbou DPH 12 % (§ 48 zák. č. 235/2004 Sb.) — záloha + závěrečná faktura, VOP, záruční podmínky.',
      obsah: HTML_SOD_12PCT,
      typSablony: 'html',
    },
    update: {
      nazev: 'SOD 12 % DPH — se zálohou',
  ...
  console.log('Hotovo: SOD 12 % DPH — se zálohou')
}
