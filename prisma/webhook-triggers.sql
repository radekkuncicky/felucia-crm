-- Triggery transakčního outboxu webhooků. Idempotentní.
-- Zdroj pravdy pro trigger DDL — migrace 20260704092500_webhook_outbox_triggers
-- obsahuje tentýž kód (pro prod migrate flow); test DB vznikají přes
-- `prisma db push`, který triggery nezná, proto je aplikují tests/setup.ts
-- a scripts/e2e.sh z tohoto souboru. Při změně uprav OBOJE.
-- Sledované entity drž v sync s WEBHOOK_EVENTS v lib/webhooks.ts.
